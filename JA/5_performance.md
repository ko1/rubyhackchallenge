...
全体的な高速化のために、JIT コンパイラ（Just-in-Time コンパイラ、実行時コンパイラ）を開発する、再帰呼び出しを軽量化するために、再帰メソッド呼び出しを、単なるジャンプへ変換する、といった工夫が考えられます（実際、この資料を書いたあと、MRI には YJIT と ZJIT という JIT コンパイラが入りました。`ruby --yjit` などで試せます）。が、それらの工夫では、たかだか数倍から数百倍の高速化しか見込めません（それだけ速くなればいいだろ、という話もありますが）。
...
* VM
  * 命令セットを検討する。
  * JIT コンパイル。現在は YJIT（Ruby 3.1 から）と ZJIT（Ruby 4.0 から）という 2 つの JIT コンパイラが Rust で書かれており、`yjit/`、`zjit/` にあります。
