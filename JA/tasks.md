...
Redmine <https://bugs.ruby-lang.org/projects/ruby-master/issues> で未解決のバグチケットを見つけ、調査してみましょう。自分が興味のある、もしくは自分が困っているチケットですと、やる気が出ると思います。
...
## エージェントと一緒に未解決チケットを漁る

Redmine には、再現手順が曖昧なまま止まっているバグチケットがたくさんあります（[(1)](1_culture.md) 参照）。
コーディングエージェントに、チケットの本文を渡して再現スクリプトを書かせる、手元でビルドした ruby で走らせる、当たりが付いたら `git bisect` を回す、といった作業は、かなりの部分を任せられます。

そのうえで、**「本当に再現したか」「その原因で合っているか」は自分で確かめて**、結果をチケットにコメントしてください。
再現手順が確定するだけでも、そのバグは半分以上解けたようなものです。

## Ractor を使ってみる

[(7) Ractor を触ってみよう](7_ractor.md) を参考に、Ractor を使った並列プログラムを書いてみましょう。
そして、うまくいかなかったところ（エラーメッセージが不親切、Ractor から使えないライブラリがあった、並列にしても速くならなかった、など）を報告してください。
実際に使ってみた報告は、いま Ractor に一番足りていないものです。
